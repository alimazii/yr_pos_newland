...
/************************************************************************/
/* 函数名：EncodeDataAndGenerateBmp
功能:将输入数据以QR算法加密后生成位图
输入参数:stDataInfo
输出参数：stBmpInfo
返回值：APP_FAIL or APP_SUCC
*/
/************************************************************************/
int EncodeDataAndGenerateBmp(DataInfo* stDataInfo, BmpInfo* stBmpInfo)
{
	int i, j;
...
char* BmpZoomIn(int ntimes)
{
	...
	for (i = 0; i < m_nSymbleSize*ntimes; ++i)
	{
		...
		{
			data[i*m_nSymbleSize*ntimes+j] = tmpbuf[j][i];//横列对调
		}	
	}
	return data;
}
